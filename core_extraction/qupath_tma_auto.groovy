// QuPath TMA核心提取脚本 (自动版本)
// 自动根据输入图片路径确定caseID和输出目录
...
// 获取命令行参数（仅接收 outputPath 与 targetSize；输入图像由 -i 提供）
def scriptArgs = this.binding.hasVariable('args') ? args : new String[0]
def outputPath = scriptArgs.length > 0 ? scriptArgs[0] : '/home/zheng/zheng/rosie_reproduce/core_extraction/tma_cores_pngs'
def targetSize = scriptArgs.length > 1 ? Double.parseDouble(scriptArgs[1]) : 1024.0
...
// 自动解析输入路径，提取caseID（去除Bio-Formats附加的 [--series, 0] 等后缀）
def inputFile = new File(path)
def filename = inputFile.getName()
def caseBase = filename
def svsIdx = caseBase.indexOf('.svs')
if (svsIdx >= 0) {
    caseBase = caseBase.substring(0, svsIdx)
}
def caseID = caseBase
